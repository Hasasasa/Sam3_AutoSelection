...
- **悬停选取**：在浏览器中使用 **decoder‑only ONNX 模型** 实现毫秒级悬停分割；解码器输出 3 个候选掩膜并按 IoU 选最优，得到完整、稳定的选区。
...
> **悬停效果优化**：解码器以 multimask 模式导出（输出 3 个候选掩膜 + 各自 IoU 分数），前端按 IoU 选出“整体物体”那个掩膜——选区更完整，且在物体内移动时更稳定（不会一动一变）。浏览器内的后处理流程也与后端 `post_process_masks` 对齐：先把**原始 logits** 上采样到显示分辨率再二值化（边缘平滑、保留凹凸结构），随后做**闭运算**补回边缘小凹口、**保留最大连通域**去噪、**填充内部空洞**，最终得到平滑且实心的掩膜。
